...
*/
class Shelf_Planner_Connector_Install_Upgrade_Deinstall_Database {
	/**
	...
	public function database_install_and_or_upgrade( $instantiated_class = null) {
		 ...
		 * 
		 * If changed '$plugin_slug = "shelf_planner_connector";', updated all the concurrences of the plugin
		 */
		$plugin_slug = 'shelf_planner_connector';
		$plugin_title = 'Shelf Planner Connector';
		...
		if ( version_compare( $database_version_installed, $database_version_upgrade) < 0 ) {
			...
			//Don't use "wp_shelf_planner_connector_some_table", use instead: ".$wpdb->prefix."shelf_planner_connector_some_table
			//The WordPress Instalation define the prefix, use '".$wpdb->prefix."shelf_planner_connector_some_table' isntead of 'wp_shelf_planner_connector_some_table'
			$wpdb->query("
				CREATE TABLE `".$wpdb->prefix.$plugin_slug."_"."table`  (
			  	) ENGINE");
			...
			$shelf_planner_connector_languages = ["en", "es"];
			update_option( $plugin_slug."_"."languages", $shelf_planner_connector_languages);
			...
			wp_schedule_event( time(), 'hourly', 'bl_shelf_planner_connector_cronjob' );
	...
	public function dabatase_erase() {
		...
		//Stop the cronjobs
		wp_clear_scheduled_hook('bl_shelf_planner_connector_cronjob');
	}
}